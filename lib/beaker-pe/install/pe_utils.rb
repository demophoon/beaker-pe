      ...
      module PEUtils
        ...
        def check_console_status_endpoint(host)
          ...
          Timeout.timeout(60) do
            match = nil
            while not match
              output = JSON.parse(on(host, "curl -s -k https://localhost:4433/status/v1/services --cert /etc/puppetlabs/puppet/ssl/certs/#{host}.pem \
                          --key /etc/puppetlabs/puppet/ssl/private_keys/#{host}/.pem \
                          --cacert /etc/puppetlabs/puppet/ssl/certs/ca/.pem", :accept_all_exit_codes => true))
